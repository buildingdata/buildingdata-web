...
@RestController
@RequestMapping(value = "/import_t_indoor_confort_suggestion")
public class Import_t_indoor_confort_suggestion {
    /**
     导入室内热环境分级的数据，对应数据库中的表为t_indoor_confort_suggestion
     */

    @Autowired
    private indoorSuggestService indoorSuggestService;

    private double flag = 0;
    int count=0;
    @ResponseBody
    @RequestMapping("/upload_t_indoor_confort_suggestion")
        ...
        try {
            if(name!=null){
                ...
                for (int i = 1; i <= RowNum; i++){
                    ...
                    indoorSuggestInfo indoorSuggestInfo=new indoorSuggestInfo();
                    ...
                    if (row != null) {
                        ...
                        if(cells[0]!=null){
                            indoorSuggestInfo.setLevel(cells[0].getStringCellValue());
                        }else{
                            buffer.add("数据集为空！！");
                        ...
                        if (cells[1]!=null){
                            indoorSuggestInfo.setPPD( cells[1].getStringCellValue());
                        }else {
                            indoorSuggestInfo.setPPD(null);
                        }

                        if(cells[2]!=null){
                            indoorSuggestInfo.setPMV(cells[2].getStringCellValue());
                        }else {
                            indoorSuggestInfo.setPMV(null);
                        }
                    ...
                    if (indoorSuggestInfo!= null) {
                        indoorSuggestService.update_t_indoor_confort_suggestion(indoorSuggestInfo);
