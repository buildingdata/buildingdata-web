...
@RestController
@RequestMapping(value = "/import_icap")
public class import_icap {
        ...
        try {
            if(name!=null){
                ...
                for (int i = 1; i <= RowNum; i++){
                    ...
                    IndoorComputeParamInfo indoorComputeParamInfo=new IndoorComputeParamInfo();
                    ...
                    if (row != null) {
                        ...
                        if (cells[0]!=null){
                            ...
                            indoorComputeParamInfo.setStationId((int)cells[0].getNumericCellValue());
                        }else{
                        ...
                        if (cells[1]!=null){
                            ...
                            indoorComputeParamInfo.setArchType(cells[1].getStringCellValue().replace("建筑",""));
                        }else{
                            buffer.add("数据集缺少重要信息！！");
                            jsonObject.put("message",buffer.toString());
                            return jsonObject;
                        ...
                        if (cells[2]!=null){
                            ...
                            indoorComputeParamInfo.setArchLevel(cells[2].getStringCellValue());
                        }else{
                            buffer.add("数据集缺少重要信息！！");
                            jsonObject.put("message",buffer.toString());
                            return jsonObject;
                        ...
                        if (cells[3]!=null){
                            ...
                            String str[]=cells[3].getStringCellValue().split("-");
                            indoorComputeParamInfo.setHotLowTemp(Double.parseDouble(str[0]));
                            indoorComputeParamInfo.setHotHighTemp(Double.parseDouble(str[1]));
                        }else{
                            indoorComputeParamInfo.setHotLowTemp(null);
                            indoorComputeParamInfo.setHotLowTemp(null);
                        }

                        if (cells[4]!=null){
                            cells[4].setCellType(CellType.STRING);//设置单元格格式
                            String str[]=cells[4].getStringCellValue().split("-");
                            indoorComputeParamInfo.setColdLowTemp(Double.parseDouble(str[0]));
                            indoorComputeParamInfo.setColdHighTemp(Double.parseDouble(str[1]));
                        }else{
                            indoorComputeParamInfo.setColdLowTemp(null);
                            indoorComputeParamInfo.setColdHighTemp(null);
                    ...
                    if (indoorComputeParamInfo!= null) {
                        indoorComputeParamService.update_icap(indoorComputeParamInfo);
