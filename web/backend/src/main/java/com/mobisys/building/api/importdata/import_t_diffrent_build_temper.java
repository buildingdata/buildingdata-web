...
@RestController
@RequestMapping(value = "/import_t_diffrent_build_temper")
public class import_t_diffrent_build_temper {
    @Autowired
    private indoorDiffBuilService indoorDiffBuilService;

    private double flag = 0;
    int count=0;
    @ResponseBody
    @RequestMapping("/upload_t_diffrent_build_temper")
        ...
        try {
            if(name!=null){
                ...
                for (int i = 1; i <= RowNum; i++){
                    ...
                    indoorDiffBuilInfo indoorDiffBuilInfo =new indoorDiffBuilInfo();
                    ...
                    if (row != null) {
                        ...
                        for (int m = 2; m < 4; m++) {
                            cells[m] = row.getCell(m);//获取单元格
                            if (cells[m]!=null){
                                cells[m].setCellType(CellType.NUMERIC);//设置单元格格式
                            }
                        }
                        ...
                        if(cells[0]!=null){
                            indoorDiffBuilInfo.setType(cells[0].getStringCellValue());
                        }else{
                        ...
                        if (cells[1]!=null){
                            indoorDiffBuilInfo.setLevel( cells[1].getStringCellValue());
                        }else {
                            buffer.add("数据集缺少重要信息！！");
                            jsonObject.put("message",buffer.toString());
                            return jsonObject;
                        ...
                        if(cells[2]!=null){
                            indoorDiffBuilInfo.setLimitLowHeatTemp(cells[2].getNumericCellValue());
                        }else {
                            indoorDiffBuilInfo.setLimitLowHeatTemp(null);
                        }
                        if(cells[3]!=null){
                            indoorDiffBuilInfo.setLimitUpHeatTemp(cells[3].getNumericCellValue());
                        }else {
                            indoorDiffBuilInfo.setLimitUpHeatTemp(null);
                        }
                    ...
                    if (indoorDiffBuilInfo!= null) {
                        indoorDiffBuilService.update_t_diffrent_build_temper(indoorDiffBuilInfo);
