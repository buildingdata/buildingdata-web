...
import com.mobisys.building.api.Thread.ExtremeMeteoroYearAirMutiThread;
...
@RestController
@RequestMapping(value = "/importextrememeteorologicalyearair")
public class ImportExtremeMeteorologicalYearAir {
    @Autowired
     ...
     * 数据表格的sheet1里面多了一列台站号
     */
    @ResponseBody
    @RequestMapping(value = "/uploadextrememeteorologicalyearair")
    public JSONObject uploadextrememeteorologicalyearair(@RequestParam("file")MultipartFile file,String name) {
                ...
                if(name!=null) {
                    ...
                    for(int i=0;i<sheetNum;i++) {
                        ...
                        for(int j=1;j<=threadNum;j++) {
                            int startRow=(j-1)*500+2;
                            int endRow=j*500;
                            if(j==threadNum) {
                                endRow=RowNum;
                            }
                            Future<Integer> future=executorService.submit(new ExtremeMeteoroYearAirMutiThread(semaphore,sheet,startRow,endRow,extremMeteoroLogicalService,countDownLatch));
                            futures.add(future);
                        }
