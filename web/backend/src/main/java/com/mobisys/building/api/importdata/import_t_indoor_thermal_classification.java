...
@RestController
@RequestMapping(value = "/import_t_indoor_thermal_classification")
public class import_t_indoor_thermal_classification {
    /**
     * 导入室内热舒适建议的数据，对应数据库中的表为t_indoor_thermal_classification
     */

    @Autowired
    private indoorHotService indoorHotService;

    private double flag = 0;
    int count=0;

    @ResponseBody
    @RequestMapping("/upload_t_indoor_thermal_classification")
        ...
        try {
            if(name!=null){
                ...
                for (int i = 1; i <= RowNum; i++){
                    ...
                    indoorHotInfo indoorHotInfo = new indoorHotInfo();
                    ...
                    if (row != null) {
                        ...
                        if(cells[0]!=null){
                            indoorHotInfo.setLevel(cells[0].getStringCellValue());
                        }else{
                            buffer.add("数据集为空！！");
                        ...
                        if(cells[1]!=null){
                            indoorHotInfo.setRequireL( cells[1].getStringCellValue());
                        }else {
                            indoorHotInfo.setRequireL(null);
                        }

                        if(cells[2]!=null){
                            indoorHotInfo.setInterpretation(cells[2].getStringCellValue());
                        }else {
                            indoorHotInfo.setInterpretation(null);
                        }
                        System.out.println("cells[0].getStringCellValue()值"+cells[0].getStringCellValue());
                    }

                    if (indoorHotInfo!= null) {
                        indoorHotService.update_t_indoor_thermal_classification(indoorHotInfo);
