...
export default {
      data(){
        return {
          ...
          provinceSelect: '北京',
          province:[],
          params:["非供暖房间保温设计室内计算参数","非空调房间隔热设计室内计算参数","供暖房间保温设计室内计算参数",
       "空调房间隔热设计室内计算参数","自然通风设计室内计算参数"],
          paramSelect:'',
          option:false,
          stationId:'54511',
          climates:'1A',
   ...
   watch: {
    form(newval, val) {
      console.log(newval)
      if(newval=="建筑热工"){
       this.params= ["非供暖房间保温设计室内计算参数","非空调房间隔热设计室内计算参数","供暖房间保温设计室内计算参数",
       "空调房间隔热设计室内计算参数","自然通风设计室内计算参数"]
      
       this.paramShow  = true
     }else {
       console.log("不能显示了",this.params)
