...
// 0 是企业
function post_register(type) {
    if (type == 0) {
        var userName = $('#name_1').val(),
            c = $('#szd_1').val(),
            w = $('#hy_1').val(),
            phone = $('#sjhm_1').val(),
            code = $('#verifyCode_front_1').val(),
            yjfk = $('#yjfk_1').val(),
            pwd = $('#pwd_1').val(),
            repwd = $('#repwd_1').val();
        var qyxm = $('#qymc').val();
        if (qyxm == "undefined" || qyxm == null || qyxm == "") {
            alert("企业名不能为空");
        ...
        /*else if (phone.match(/^1[3,5,7,8]\d{9}$/g) == null) {
            ...
            alert("请选择行业");
        } else if (yjfk == "undefined" || userName == null || userName == "") {
            alert("请输入详细情况");
        }
        else if (hex_md5(code) == verificationCode) {
            var city = $("#szd").find("option:selected").text();
            var work = $("#hy").find("option:selected").text();
            $.ajax({
                url: "/apis/building/user/register?phone=" + phone + "&&name=" + userName + "&&city=" + city + "&&work=" + work + "&&yjfk=" + yjfk + "&&type=" + type + "&&qyxm=" + qyxm + "&&pwd=" + pwd,
                type: "get",
                contentType: "application/json",
                dataType: "json",
                success: function (res) {
    ...
    // 1 是个人
    if (type == 1) {
        var userName = $('#name_2').val(),
            c = $('#szd_2').val(),
            w = $('#hy_2').val(),
            phone = $('#sjhm_2').val(),
            code = $('#verifyCode_front_2').val(),
            yjfk = $('#yjfk_2').val(),
            pwd = $('#pwd_2').val(),
            repwd = $('#repwd_2').val();
    ...
    console.log("填写的code md5是：", hex_md5(code));
...
function sendMessage() {
    var phoneNum = $('#sjhm_1').val();
    var count = 60;
    $('#sjhm_1').click(function () {
    ...
    //开始计时  
    var dom = document.getElementById('vcode_send_btn');
    dom.setAttribute("style", "width: 112px;margin-left: 10px;border-radius: 5px;opacity: 0.6;cursor: not-allowed;");
    $("#vcode_send_btn").attr('disabled', 'disabled');
    $("#vcode_send_btn").html("倒计时" + count + "秒");
    var timer = setInterval(function () {
        count--;
        $("#vcode_send_btn").html("倒计时" + count + "秒");
        if (count == 0) {
            clearInterval(timer);
            $("#vcode_send_btn").attr("disabled", false);//启用按钮  
            dom.setAttribute("style", "width: 112px;margin-left: 10px;border-radius: 5px;");
            $("#vcode_send_btn").html("重新发送");
...
function sendMessage2() {
    var phoneNum = $('#sjhm_2').val();
    var count = 60;
    $('#sjhm_2').click(function () {
        var reg_phone = /1\d{10}/;
        if (!reg_phone.test(phoneNum)) {   //验证手机是否符合格式
            alert("请填写手机号");
            return false;
    ...
    var dom = document.getElementById('vcode_send_btn');
    dom.setAttribute("style", "width: 112px;margin-left: 10px;border-radius: 5px;opacity: 0.6;cursor: not-allowed;");
    $("#vcode_send_btn_2").attr('disabled', 'disabled');
    $("#vcode_send_btn_2").html("倒计时" + count + "秒");
    var timer = setInterval(function () {
        count--;
        $("#vcode_send_btn_2").html("倒计时" + count + "秒");
        if (count == 0) {
            clearInterval(timer);
            $("#vcode_send_btn_2").attr("disabled", false);//启用按钮  
            dom.setAttribute("style", "width: 112px;margin-left: 10px;border-radius: 5px;");
            $("#vcode_send_btn_2").html("重新发送");
            code = "";//清除验证码。如果不清除，过时间后，输入收到的验证码依然有效 
        }
    }, 1000);
    $.ajax({
